...
export class LambdaPrintService {
  ...
  hasMacro( term ): boolean {
    // ToDo
    if ( this.evaluator.isAlphaEqual( term, this.parser.parse('PLUS') ) ) return true;
    if ( this.evaluator.isAlphaEqual( term, this.parser.parse('SUCC') ) ) return true;
    if ( this.macro.isNumber( term ) ) return true;
    return false;
  }

  toMacroString( term ): string {
    if ( this.evaluator.isAlphaEqual( term, this.parser.parse('PLUS') ) ) return '+';
    if ( this.evaluator.isAlphaEqual( term, this.parser.parse('SUCC') ) ) return 'SUCC';
    if ( this.macro.isNumber( term ) ) return this.macro.toNumber( term ).toString();
    return '[ERROR]';
  }
